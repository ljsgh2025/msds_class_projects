...
Here we read in the data provided in class. This data is from John Hopkins University and downloaded from github.com. The data contains information such as total cases and deaths for each day of the COVID pandemic. Data is provided for many global countries and regions as well as the US by state and county. I will be using both `library(tidyverse)` and `library(lubridate)` we used both of these in class so I hope everyone will be OK.
...
I created a model here to check my assumption that number of deaths is related to number of cases. I think this is an intuitive assumption. However I wanted to check to see if it holds true. I think this model shows that these two variables are closely related. An r squared value of 0.9093 is pretty predictive. Plotted we see the linear model in red with the data from US states in black points.

```{r check assumtions with model, echo=TRUE}
mod_deaths_cases = lm(deaths ~ cases, data = US_by_state)

summary(mod_deaths_cases)
US_by_state_mod = US_by_state %>%
  mutate(pred_deaths = predict(mod_deaths_cases))

US_by_state_mod %>%
  ggplot() +
  geom_point(aes(x = cases, y = deaths)) +
  geom_line(aes(x = cases, y = pred_deaths, color = "red"))
```
...
## More Data filtering and cleaning
...
Linear model predicting death rate depending on political vote. R-squared of 0.016 is not very predictive. This is a valid model. By showing how these two variables are not related or closely correlated we learn something about our data.
...
## Model 2 Visual
  ...
  Looking at this visual it appears the linear model does not fit the data very well. The means and standard deviations of each political vote category appear to be the same. I think part of this is due to the extreme variations over time. In fact, if you look back to visual 2, most of the variation is due to change over time.
...
I was able to show that number of deaths id closely related to total number of cases with a linear model. I further used that data to support my calculation of deaths per 1,000 cases. This calculation, `deaths_per_k_cases` was used in comparing a number of things. first being deaths rates in different states (visual 1). Then I added some new data about political voting and used these together to plot a time series of average death rate per 1,000 cases with two categories, GOP vote or DEM vote (visual 2). 

I then attempted to make a Linear model predicting death rate depending on political vote (model 2). This however was not very predictive and ended up not showing much. I used this information to create visual 3, a histogram of death rates by county shaded by political vote. This gives insight into why model 2 failed to produce results.
